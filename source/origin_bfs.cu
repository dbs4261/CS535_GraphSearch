...
void BFS_host(int source, int *edges, int *dest, int *label, int edges_num, int vertex_num){
  ...
  CHECK_ERROR(cudaMalloc((void**)&edges_d, (vertex_num + 1) * sizeof(int)));
  CHECK_ERROR(cudaMalloc((void**)&dest_d, edges_num * sizeof(int)));
  CHECK_ERROR(cudaMalloc((void**)&label_d, vertex_num * sizeof(int)));
  CHECK_ERROR(cudaMalloc((void**)&visited_d, vertex_num * sizeof(int)));

  cudaMemcpy(edges_d, edges, (edges_num * sizeof(int)), cudaMemcpyHostToDevice);
  cudaMemcpy(dest_d, dest, ((vertex_num + 1) * sizeof(int)), cudaMemcpyHostToDevice);
  cudaMemcpy(label_d, label, (vertex_num * sizeof(int)), cudaMemcpyHostToDevice);

  int *frontier_d, *c_frontier_tail_d, *p_frontier_tail_d;
  CHECK_ERROR(cudaMalloc((void**)&frontier_d, 2 * MAX_FRONTIER_SIZE * sizeof(int)));
  int *c_frontier_d = &frontier_d[0];
  int *p_frontier_d = &frontier_d[MAX_FRONTIER_SIZE];
  
  cudaMemset(visited_d, 0, vertex_num * sizeof(int));
  printf("1\n");
